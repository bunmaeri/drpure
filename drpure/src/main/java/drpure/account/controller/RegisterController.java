package drpure.account.controller;
...
import drpure.account.service.LoginService;
...
import drpure.common.email.ForgotPasswordEmail;
...
import drpure.language.RegisterLanguage;
...
//			html2.append("\n개인통관고유번호: ").append(requisition_id);
			...
			*/
			...
			StringBuffer join_path = new StringBuffer();
			Map<String,Object> joinMap = (Map<String,Object>)(loginService.joinPathName(customerMap)).get("map");
			join_path.append(joinMap.get("join_path_name"));
			if(joinMap.get("is_input").equals("1") && !join_path_etc.equals("")) {
				join_path.append(" ("+join_path_etc+")");
			}
			Map<String,Object> officeMap = new HashMap<String,Object>();
			officeMap.put("customer_name", customer_name);
			officeMap.put("customer_group", nameMap.get("name"));
			officeMap.put("email", email);
			officeMap.put("telephone", telephone);
			officeMap.put("join_path", join_path.toString());
	    	String html2 = RegisterOfficeEmail.getHtml(officeMap);
